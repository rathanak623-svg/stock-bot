...
async function processBulkAddItems(entries, actorCtx, deptCtx) {
  ...
  for (const line of entries) {
    ...
    const result = await withStockSheetWriteLock(async () => withLock(`stock:${deptCtx.stockSheet}:${itemName}`, async () => {
      ...
      await runNonCriticalTask('Append report error:', () =>
        appendReport('ADD_ITEM_BULK', `By=${actorCtx.actor}, Department=${deptCtx.department}, Item=${itemName}, MinAlert=${minAlert}, Unit=${unit}`)
      );
  ...
  for (const line of entries) {
    ...
    const result = await withStockSheetWriteLock(async () => withLock(`stock:${deptCtx.stockSheet}:${itemName}`, async () => {
      ...
      await Promise.all([
        ...
        runNonCriticalTask('Append report error:', () =>
          appendReport(`${mode}_BULK`, `By=${actorCtx.actor}, Department=${deptCtx.department}, Item=${r[0]}, Qty=${qty}, Before=${oldBalance}, After=${newBalance}, Note=${note}`)
        )
      ]);
