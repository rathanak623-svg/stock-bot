...
function buildQuickActionsInlineKeyboard(role = 'guest') {
  ...
  if (role === 'super_admin' || role === 'admin') {
    ...
    rows.push(
      ...
      [{ text: '🗂 ALL STOCK', callback_data: 'QA|ALLSTOCK' }, { text: '📈 DASHBOARD', callback_data: 'QA|DASHBOARD' }],
      [{ text: '📋 MENU', callback_data: 'QA|MENU' }]
    );
  } else {
    rows.push(
      [{ text: '📊 STOCK', callback_data: 'QA|STOCK' }, { text: '🚨 LOW STOCK', callback_data: 'QA|LOWSTOCK' }],
      [{ text: '🗂 ALL STOCK', callback_data: 'QA|ALLSTOCK' }, { text: '📋 MENU', callback_data: 'QA|MENU' }]
    );
  }
...
async function getData() {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: SHEET_RANGES.Stock });
  return res.data.values || [];
}
async function getLogs() {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: SHEET_RANGES.Logs });
  return res.data.values || [];
}
...
async function getPendingActions() {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: SHEET_RANGES.PendingActions });
  return res.data.values || [];
}
async function getProcessedMessages() {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: SHEET_RANGES.ProcessedMessages });
  return res.data.values || [];
}
...
async function addAllowedChat(chatId, chatTitle, chatType) {
  return withAllowedChatsLock(async () => {
    ...
    await sheets.spreadsheets.values.append({
      ...
      valueInputOption: 'RAW',
      requestBody: { values: [[String(chatId), clean(chatTitle), clean(chatType), nowIso()]] }
    });
...
/**************** BULK HELPERS ****************/
async function processBulkAddItems(entries, actorCtx) {
  const results = [];
  for (const line of entries) {
    ...
    const result = await withStockSheetWriteLock(async () => withLock(`stock:${itemName}`, async () => {
      const data = await getData();
      const existing = findRowIndex(data, itemName);
      if (existing !== -1) return `⚠️ Already exists: ${itemName}`;
  ...
  for (const line of entries) {
    ...
    const result = await withStockSheetWriteLock(async () => withLock(`stock:${itemName}`, async () => {
      ...
      if (mode === 'IN') {
        ...
        if (qty > oldBalance) return `❌ Not enough stock: ${itemName} (Balance ${oldBalance} ${unit})`;
      ...
      if (mode === 'OUT') await checkAndSendLowStockAlert(actorCtx.chatId, updated);
...
async function buildHealthMessage() {
  const [data, logs, pending, processed, roles, allowedChats, groupSettings] = await Promise.all([
    getData(), getLogs(), getPendingActions(), getProcessedMessages(),
...
async function handleCommand(msg) {
  ...
  try {
    ...
    if (command === '/additemsbulk') {
      ...
      const results = await processBulkAddItems(entries, actorCtx);
    ...
    if (command === '/in') {
      ...
      return withStockSheetWriteLock(async () => withLock(`stock:${itemName}`, async () => {
        ...
        await Promise.all([
          runNonCriticalTask('Append log error:', () =>
            appendLog([nowIso(), 'IN', r[0], qty, oldBalance, newBalance, unit, actorCtx.chatId, actorCtx.chatTitle, actor, role, note])
          ),
          runNonCriticalTask('Append report error:', () =>
    ...
    if (command === '/out') {
      ...
      return withStockSheetWriteLock(async () => withLock(`stock:${itemName}`, async () => {
        ...
        await Promise.all([
          runNonCriticalTask('Append log error:', () =>
            appendLog([nowIso(), 'OUT', r[0], qty, oldBalance, newBalance, unit, actorCtx.chatId, actorCtx.chatTitle, actor, role, note])
          ),
          runNonCriticalTask('Append report error:', () =>
    ...
    if (command === '/deleteitem') {
      ...
      return withStockSheetWriteLock(async () => withLock(`stock:${itemName}`, async () => {
        ...
        const pending = await createPendingAction(username, actorCtx.userId, chatId, 'deleteitem', { itemName });
        return sendMessage(
          chatId,
